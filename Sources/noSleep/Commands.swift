...
func cmdStatus() {
    ...
    let (_, listCode) = run("/bin/launchctl", "list", LABEL, suppressStderr: true)
    print("launchd: \(listCode == 0 ? "LOADED" : "NOT loaded")")
}
...
func cmdStart() {
    ...
    run("/bin/launchctl", "enable", "gui/\(UID)/\(LABEL)", captureOutput: false)
    let (_, bootstrapCode) = run("/bin/launchctl", "bootstrap", "gui/\(UID)", PLIST_PATH)
    if bootstrapCode != 0 {
...
func cmdStop() {
    ...
    run("/bin/launchctl", "bootout", "gui/\(UID)", PLIST_PATH, suppressStderr: true, captureOutput: false)
    run("/bin/launchctl", "disable", "gui/\(UID)/\(LABEL)", suppressStderr: true, captureOutput: false)
...
func cmdDoctor() {
    ...
    let (_, listCode) = run("/bin/launchctl", "list", LABEL, suppressStderr: true)
    let launchdStatus = listCode == 0 ? "Loaded" : "Not loaded"
